...
// Regex variables
var robotsTxtRegex = regexp.MustCompile(".*llow: ")
...
var awsBucketDirRegex = regexp.MustCompile(`((http|https)://s3\.(.*)\.amazonaws.com)/[a-z0-9\.-]{3,63}`)
var awsBucketSubdomainRegex = regexp.MustCompile(`((http|https)://[a-z0-9\.-]{3,63}\.s3\.amazonaws\.com)`)
...
func (c *Collector) Crawl(url string) ([]*http.Request, error) {
	...
	if c.conf.IncludeBuckets || c.conf.IncludeAll {
		// find and print all the possible buckets within the response we have already obtained.
		// findBucketsFunc will NOT send the crawler off on this bucket
		c.colly.OnResponse(func(r *colly.Response) {
			c.findBucketsFunc(r.Body, &buckets)
		})
	}
...
func (c *Collector) findBucketsFunc(body []byte, buckets *sync.Map) {
	found := awsBucketDirRegex.FindAllString(string(body), -1)                        // i.e, https://s3.us-east-2.amazonaws.com/bucketname
	found = append(found, awsBucketSubdomainRegex.FindAllString(string(body), -1)...) // i.e https://bucketname.s3.amazonaws.com

	for _, link := range found {
		if _, exists := buckets.Load(link); exists {
			continue
		}
		if link == "" {
			continue
...
// recordWithoutChecks exists for logging findings that do not need scope checks as it will always fail (i.e s3)
// note: we don't pass in reqsMade as we never want to crawl incase the target does not own the bucket (3rd party, etc)
func (c *Collector) recordWithoutChecks(tag aurora.Value, u string) bool {
	c.colorPrint(tag, u, c.conf.Plain)
	return true
}
...
func (c *Collector) parseRobots(url string, reqsMade *syncList) {
	...
	for _, line := range lines {
		if robotsTxtRegex.MatchString(line) {
			urlstring := strings.TrimLeft(robotsTxtRegex.ReplaceAllString(line, ""), " ")
